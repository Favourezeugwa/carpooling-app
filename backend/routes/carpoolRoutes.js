...
const { sendInvitationEmail, sendNotificationEmail } = require("../emailservice");

const router = express.Router();

// Create Carpool Route
router.post('/create-carpool', authenticateUser, async (req, res) => {
  const { carpoolName, pickupLocation, dropoffLocation, dateTime, role, carType, maxCarpoolers, estimatedCost, specialNotes, invitations } = req.body;

  try {
    // Validate required fields
    if (!carpoolName || !pickupLocation || !dropoffLocation || !dateTime || !role || !maxCarpoolers) {
    ...
    if (existingCarpool) {
      return res.status(400).json({ message: "A carpool with these details already exists." });
    }
...
router.put('/update-carpool/:carpoolId', authenticateUser, async (req, res) => {
  ...
  try {
    ...
    const carpool = await Carpool.findOne({ _id: carpoolId, creator: req.user._id }); // Ensure only the creator can edit
    if (!carpool) {
      return res.status(404).json({ message: "Carpool not found or you're not authorized to edit this carpool" });
    }
...
router.post('/join-carpool/:carpoolId', authenticateUser, async (req, res) => {
  ...
  try {
    ...
    const isAlreadyInCarpool = carpool.carpoolers.some(carpooler => carpooler.user.equals(req.user._id));
...
router.post('/leave-carpool/:carpoolId', authenticateUser, async (req, res) => {
  ...
  try {
    ...
    if (carpool.creator.equals(req.user._id)) {
      ...
      return res.status(200).json({ message: "Carpool deleted as creator left" });
    }

    // If the user is a driver, remove them from the drivers array
    carpool.drivers = carpool.drivers.filter(driver => !driver.user.equals(req.user._id));

    // If the user is a rider, remove them from the riders array
    carpool.riders = carpool.riders.filter(rider => !rider.equals(req.user._id));
...
router.delete('/delete-carpool/:carpoolId', authenticateUser, async (req, res) => {
  const { carpoolId } = req.params;
...
router.post('/invite-to-carpool/:carpoolId', authenticateUser, async (req, res) => {
  const { carpoolId } = req.params;
  ...
  try {
    ...
    if (!carpool.creator.equals(req.user._id)) {
      return res.status(403).json({ message: "You are not authorized to send invitations for this carpool" });
    }

    // Filter out duplicate invitations
    const newInvitations = invitations.filter(
      invite => !carpool.invitations.some(existingInvite => existingInvite.email === invite.email)
    );

    // Check if adding these invitations would exceed maxCarpoolers
    const totalCarpoolersIfAdded = carpool.currentCarpoolers + newInvitations.length;
    ...
    res.status(200).json({ message: "Invitations sent successfully", carpool });
  } catch (error) {
...
router.post('/accept-invite/:carpoolId', authenticateUser, async (req, res) => {
  ...
  try {
    ...
    const invitation = carpool.invitations.find(invite => invite.email === req.user.email);
    if (!invitation) {
    ...
    carpool.invitations = carpool.invitations.filter(invite => invite.email !== req.user.email);
